    ...
    $.mobiscroll.classes.Scroller = function (el, settings) {
        ...
        function generateWheelItems(i) {
            ...
            $.each(values, function (j, v) {
                ...
                html += '<div role="option" aria-selected="false" class="dw-li dw-v" data-val="' + keys[j] + '"' + (labels[j] ? ' aria-label="' + labels[j] + '"' : '') + ' style="height:' + itemHeight*2 + 'px;line-height:' + itemHeight*2 + 'px;">' +
        ...
        function scroll(t, index, val, time, active) {
            ...
            if (has3d) {
                style[pr + 'Transform'] = 'translate3d(0,' + px*2 + 'px,0)';
            } else {
                style.top = px + 'px';
            }
                ...
                $.each(wg, function (j, w) { // Wheels
                    ...
                    html += '<' + (hasFlex ? 'div' : 'td') + ' class="dwfl"' + ' style="' +
                                ...
                                '<div tabindex="0" aria-live="off" aria-label="' + lbl + '" role="listbox" class="dwww">' +
                                    '<div class="dww" style="height:' + (s.rows * itemHeight*2) + 'px;">' +
                                        '<div class="dw-ul">';

                    // Create wheel values
                    html += generateWheelItems(l) +
                        '</div></div><div class="dwwo"></div></div><div class="dwwol"' +
                        (s.selectedLineHeight ? ' style="height:' + itemHeight*2 + 'px;margin-top:-' + (itemHeight / 2 + (s.selectedLineBorder || 0)) + 'px;"' : '') + '></div></div>' +
                        (hasFlex ? '</div>' : '</td>');
